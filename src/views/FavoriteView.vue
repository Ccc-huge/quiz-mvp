<template>
  <div class="page">
    <header class="bar">
      <div class="bar-inner">
        <button class="back" @click="$router.push('/modes')">← 返回</button>
        <div class="title">
          <div class="h">我的收藏</div>
          <div class="s">共收藏 {{ items.length }} 道题目</div>
        </div>
      </div>
    </header>
      ...
      <div class="question-list">
        <div class="question-card" v-for="q in filteredItems" :key="q.id">
          ...
          <div class="card-actions">
            <button class="action-btn" @click="removeFavorite(q.id)">取消收藏</button>
            <button class="action-btn primary" @click="practiceSingle(q)">练习此题</button>
          </div>
        </div>
      </div>
    </main>
    ...
    <main class="main empty" v-else>
      ...
      <div class="empty-sub">在练习中收藏的题目将显示在这里</div>
  ...
  <div class="modal-overlay" v-if="selectedQuestion" @click.self="selectedQuestion = null">
    <div class="modal">
      ...
      <div class="modal-footer">
        <button class="btn" @click="removeFavorite(selectedQuestion.id)">取消收藏</button>
...
function refresh() {
  const favIds = getItem('FAVORITE_QUESTIONS', []);
  const bank = getCurrentQuestions();
  items.value = bank.filter(q => favIds.includes(q.id));
}
...
.bar-inner { max-width: 800px; margin: 0 auto; padding: 14px 16px; display: flex; gap: 12px; align-items: center; }
...
.action-btn { padding: 6px 12px; border-radius: 6px; border: 1px solid #ddd; background: #fff; font-size: 12px; cursor: pointer; transition: all 0.2s; }
...
.empty-title { font-size: 20px; font-weight: 700; color: #333; margin-bottom: 8px; }
